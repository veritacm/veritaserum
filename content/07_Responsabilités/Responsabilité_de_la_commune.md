---
title: Responsabilité de la commune – Cadre juridique (sans préjuger des faits)
date: 2026-01-05
type: responsabilités
publicable: true
topic: "veritaserum_crans_montana"
status: "active"
---

# Responsabilité de la commune – Cadre juridique (sans préjuger des faits)

## Objet
TODO: Expliquer les formes de responsabilité possibles, sans les attribuer.

## Responsabilité administrative (exécution des tâches)
- TODO: Devoir de diligence, contrôle, documentation.

## Responsabilité civile publique (principes)
- TODO: Dommage, causalité, faute, responsabilité des collectivités (à documenter).

## Responsabilité pénale (principes généraux)
- TODO: Négligence, organes, conditions de mise en cause.

## Limites et prudence
- TODO: Ce qui relève du juge, du MP, du contrôle administratif.

## Liens internes
- [[Lois_applicables]]
- [[Procédures_de_contrôle]]
- [[Commune_de_Crans-Montana]]
- [[Questions_factuelles_à_clarifier]]
